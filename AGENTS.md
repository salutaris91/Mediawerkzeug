...
- **Executive-Recommendation-Pflicht:** Wenn Alex eine Entscheidung treffen soll, stelle der Frage immer eine begründete Empfehlung voran: Empfehlung (bevorzugte Option), Kurzbegründung (1-3 Sätze), Trade-off/Risiko (wichtigster Nachteil/Unsicherheit) und Alex-Entscheidung (konkrete Frage). Triff Entscheidungen nie stellvertretend. Fehlen Infos für eine Empfehlung, lautet diese explizit „erst klären" mit Angabe der fehlenden Info.
...
Freigabepunkte sind: Plan-Freigabe, PR-Merge, ausgelöste `design-frage` /
`sicherheits-frage` sowie jede explizit an Alex gerichtete Frage.

1. Freigabepunkte beantwortet ausschließlich Alex. Agenten geben Antworten
   darauf NIEMALS stellvertretend — nicht einsetzen, nicht simulieren, nicht
   in Sessions oder Werkzeuge injizieren. Auch nicht mit plausiblen,
   empfohlenen oder zuvor diskutierten Werten. Auch nicht bei Zeitdruck,
   blockierten Kanälen oder "es war ja ohnehin klar".
2. Empfehlungen abzugeben ist erwünscht; die Entscheidung einzusetzen ist es nie.
3. Ist ein Freigabepunkt erreicht und Alex nicht verfügbar: an dieser Stelle
   stoppen, Zustand dokumentieren, an nicht-blockierten Stellen weiterarbeiten
   oder sauber beenden.
4. Im Zweifel gilt: Es ist ein Freigabepunkt.
...
- Produkte entstehen KI-unterstützt: KI sorgt für schnellen Implementierungsdurchsatz, wodurch sich Alex' Rolle auf Produktentscheidungen, Review, Qualitätskontrolle, Architektur, Akzeptanzkriterien und Freigabepunkte verlagert.
- Zeitschätzungen und Meilensteine dürfen nicht klassische Solo-Entwickler-Timelines annehmen, sondern müssen KI-ausführbare Coding-Arbeit (schnell) und menschliche Review-/Entscheidungszeit (Engpass) zwingend getrennt betrachten.
